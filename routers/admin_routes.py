...
from services.complaint_service import serialize_admin_complaint
...
def get_all_complaints(
    ...
    if category_filter:
        query = query.filter(Complaint.ai_category == category_filter)
...
def get_analytics(
    ...
    category_counts = {}
    categories = db.query(Complaint.ai_category).distinct().all()
    for (category,) in categories:
        if category:
            category_counts[category] = db.query(Complaint).filter(
                Complaint.ai_category == category
            ).count()

    issue_type_counts = {}
    issue_types = db.query(Complaint.issue_type).distinct().all()
    for (issue_type,) in issue_types:
        if issue_type:
            issue_type_counts[issue_type] = db.query(Complaint).filter(
